  ...
  return (
    ...
    >
      <article className="text-center mt-12 lg:mt-0 ">
        <h2 className="text-[18px] 2xl:text-[24px] font-bold">
          Nues
          <span className="relative pt-2 border-t-4 border-gradient-violet">
            tros servi
          </span>
          cios
        </h2>
        <p
          className={`mt-2 px-8 py-2 text-[14px] 2xl:text-[16px] ${poppins.className}`}
        >
          Principales servicios que ofrecemos y desarrollamos para nuestros
          clientes
        </p>
      </article>

      <section
        className={`grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6 w-full sm:w-full md:w-[80%] md:mx-auto 2xl:w-[70%] mt-12 ${poppins.className}`}
      >
        <div className="flex flex-col lg:flex-row items-center justify-center bg-white text-black font-bold text-center gap-1 py-8 px-4 border-t-4 border-[#D35CB0] rounded">
          ...
          <h3 className="text-[10px]   sm:text-[12px] lg:text-[12px] uppercase text-center">
            ESTRATÉGIA DIGITAL
          </h3>
        </div>
